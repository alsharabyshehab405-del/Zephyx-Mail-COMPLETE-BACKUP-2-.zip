...
export default function Login() {
  ...
  const onSubmit = (values: z.infer<typeof loginSchema>) => {
    ...
    loginMutation.mutate(credentials, {
      ...
      onError: (error: unknown) => {
        ...
        toast({ title: t("common.error"), description: t("login.invalidCredentials"), variant: "destructive" });
      },
    });
  };
  ...
  const onVerifyTwoFactor = () => {
    ...
    verifyMutation.mutate({ challengeToken, code: twoFactorCode.trim() }, {
      ...
      onError: (error: unknown) => {
        setTwoFactorCode("");
        toast({ title: t("common.error"), description: getApiErrorMessage(error, t("login.invalidVerificationCode")), variant: "destructive" });
      },
    });
  };
          ...
          {challengeToken ? (
            <div className="mt-6 space-y-5">
              ...
              <Button className="w-full" onClick={onVerifyTwoFactor} disabled={verifyMutation.isPending || twoFactorCode.trim().length < 6}>
                {verifyMutation.isPending ? t("common.loading") : t("login.verifyAndSignIn")}
              </Button>
