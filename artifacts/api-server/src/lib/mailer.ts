...
class DevMailer implements Mailer {
  ...
  constructor() {
    const nodemailer = localRequire("nodemailer") as NodemailerLike;
    this.from = process.env["SMTP_FROM"]?.trim() || "Zephyx Mail <noreply@zephyx.local>";
    // Nodemailer's stream transport generates a full MIME message (with
    // attachments encoded as base64 MIME parts) without needing an SMTP
    // server.  This lets us verify attachment support in development.
    this.transporter = nodemailer.createTransport({
...
<p>If you did not create this account, you can safely ignore this email.</p>`,
    ...
    logger.info({ to, messageId: info.messageId }, "Dev mailer captured verification email");
  }
...
<p>If you did not request this, you can safely ignore this email.</p>`,
    ...
    logger.info({ to, messageId: info.messageId }, "Dev mailer captured password-reset email");
  }
    ...
    logger.info(
      {
        ...
        mimeSize: info.raw?.length ?? 0,
      },
      "Dev mailer captured outgoing email (stream transport; attachments encoded as MIME base64)",
    );
  }
}
...
export function getMailer(): Mailer {
  ...
  try {
    ...
    logger.error({ err }, "SMTP transport could not be initialized; falling back to dev mailer");
    mailer = new DevMailer();
  }
  return mailer;
}
