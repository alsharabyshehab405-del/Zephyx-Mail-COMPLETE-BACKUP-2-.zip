...
// Browsers and OS MIME databases sometimes report non-standard aliases (e.g.
// "image/jpg" instead of "image/jpeg"). Map these to canonical forms so the
// signature comparison does not reject a valid file whose declared MIME is an
// alias of the detected type.
...
export function assertSafeAttachment(contents: Buffer, declaredMime: string | undefined, filename: string): string {
  ...
  const declaredCanonical = MIME_ALIASES[declared] ?? declared;
  const textDeclared = declared === "text/plain" || declared === "text/csv" || declared === "application/csv";
  if (declared && declaredCanonical !== "application/octet-stream" && declaredCanonical !== detected && !(textDeclared && detected === "text/plain")) throw Object.assign(new Error("Attachment MIME does not match its file signature"), { statusCode: 415 });
...
export async function scanAttachment(contents: Buffer, filename: string): Promise<"clean"> {
  if (!attachmentScanningEnabled()) {
    if (process.env.NODE_ENV === "development") return "clean";
    throw Object.assign(new Error("Attachment uploads are disabled until malware scanning is configured"), { statusCode: 503 });
  }
