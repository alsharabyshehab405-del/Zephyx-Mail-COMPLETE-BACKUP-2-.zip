...
const DEV_STORAGE_DIR = process.env["DEV_ATTACHMENT_STORAGE_DIR"]?.trim() || "/data/attachments";

class DevFileStorage implements AppStorageClient {
  async init(): Promise<void> {
    await fs.mkdir(DEV_STORAGE_DIR, { recursive: true });
  }

  private filePath(objectName: string): string {
    return path.join(DEV_STORAGE_DIR, ...objectName.split("/"));
  }

  async uploadFromText(objectName: string, contents: string): Promise<StorageResult<null>> {
    try {
  ...
  async downloadAsText(objectName: string): Promise<StorageResult<string>> {
    try {
      const contents = await fs.readFile(this.filePath(objectName), "utf8");
  ...
  async exists(objectName: string): Promise<StorageResult<boolean>> {
    try {
      await fs.access(this.filePath(objectName));
      return { ok: true, value: true };
    } catch {
  ...
  async delete(objectName: string): Promise<StorageResult<null>> {
    try {
      await fs.unlink(this.filePath(objectName));
      return { ok: true, value: null };
    } catch (error) {
      return { ok: false, error };
    }
